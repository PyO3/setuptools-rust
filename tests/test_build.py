...
def test_adjusted_local_rust_target_windows_msvc():
    with mock.patch(
        "setuptools_rust.rustc_info.get_rust_target_info", lambda _plat_name, _env: ["target_env=msvc"]
    ):
...
def test_adjusted_local_rust_target_windows_gnu():
    with mock.patch(
        "setuptools_rust.rustc_info.get_rust_target_info", lambda _plat_name, _env: ["target_env=gnu"]
    ):
