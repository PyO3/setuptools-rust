...
class build_rust(RustCommand):
    ...
    def _detect_rust_target(
        self, forced_target_triple: Optional[str], env: Optional[Env]
    ) -> Optional[str]:
        assert self.plat_name is not None
...
def _prepare_build_environment(env: Optional[Env]) -> Dict[str, str]:
    ...
    env = env.env.copy() if env else os.environ.copy()
    env.update(
        {
            # disables rust's pkg-config seeking for specified packages,
            # which causes pythonXX-sys to fall back to detecting the
            # interpreter from the path.
            "PATH": os.path.join(bindir, env.get("PATH", "")),
            "PYTHON_SYS_EXECUTABLE": env.get("PYTHON_SYS_EXECUTABLE", executable),
            "PYO3_PYTHON": env.get("PYO3_PYTHON", executable),
        }
    )
    return env
...
def _adjusted_local_rust_target(plat_name: str, env: Optional[Env]) -> Optional[str]:
